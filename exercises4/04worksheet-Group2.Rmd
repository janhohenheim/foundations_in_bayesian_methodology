...
MCMC_MH <- function(s_alpha, s_beta){
  ...
  for(i in -n.burnin:(n.iter*n.thin)){
    ...
    if(log(runif(1)) <= logacc){
      ...
      if(i > 0){ # count only after burnin-period
...
## Plot results:
  ...
  print(glue("Acceptance rate alpha: {formatC( acceptance_alpha , 3)} "))
  print(glue("Acceptance rate beta: {formatC( acceptance_beta , 3)} "))
