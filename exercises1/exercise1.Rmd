...
The law of total expectation states that if \( X \) and \( Y \) are random variables on the same probability space, it is E(X)=E(E(X|Y)), and similarly, the law of total variance states that Var(Y) = E(Var(Y|X)) + Var(E(Y|X)).
Since \( \mu \) and \( Y \) are both normally distributed, it is
...
When lambda goes towards zero, the variance of the prior approaches infinity and therefore the prior will become an uninformative prior. This 
...
The posterior of H0 is higher than its prior.
This is because the two models started out with equal priors (50/50), but the likelihood of seeing y=1 is higher under H0 than under H1. 
